...
/*   Updated: 2022/02/26 17:22:24 by hvan-hov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	child(t_ppx *ppx, char **argv, char **envp, int i)
{
	...
	if (!ppx->pid)
	{
		...
		{
			dup2(ppx->infile, 0);
			dup2(ppx->fd[1], 1); // write into this end of the pipe
		}
		...
		{
			dup2(ppx->fd[2 * i - 2], 0); // fd[0] // fd[2] // fd[4] --> reading from the prev pipe
			dup2(ppx->fd[2 * i + 1], 1); // fd[3] // fd[5] // fd[7] --> writing to the next pipe
		}
		close_pipes(ppx);
		ppx->cmd_args = ft_split(argv[2 + i], ' ');
