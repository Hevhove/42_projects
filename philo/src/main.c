...
/*   Updated: 2022/03/27 16:59:13 by hvan-hov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	philo_eat(t_philo *phil, t_philo *next_phil)
{
	...
	usleep(phil->data->t_eat * 1000); // problem with usleep?
...
void	philo_sleep(t_philo *phil)
{
	...
	usleep(phil->data->t_sleep * 1000); // protect from edge cases -> if dies while sleeping, what happens
...
void	*routine(void *arg)
{
	...
	while (phil->data->state == RUNNING)
	{
		...
		if (phil->data->must_check == 1) // move to main thread
		{
